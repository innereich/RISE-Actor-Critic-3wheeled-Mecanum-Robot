...
%     ro = 0.1; % External disturbance
    % ro = -0.1
    Q = 10*eye(6); % Positive-definite matrix
...
%% Initial Conditions
    ...
    %q{1} =[0.5;0.1;0.2];
    upsilon{1} = [0.0;0;0];
    W_c{1} = 500*ones(n_NN,1);
    W_a{1} = 5*ones(n_NN,1);
    gamma_upper{1} = 10*eye(n_NN);
    nominal_E = 0;
%% System simulation
for i = 1:size(t,2)
%% Noise
...
% % polygon horizontal trajectory
%     
%     q_r{i} = [a*(Rd+cos(h*w*t(i)))*cos(w*t(i)) a*(Rd+cos(h*w*t(i)))*sin(w*t(i)) 0]';
%     q_r_dot{i} = [cos(t(i)) 2*cos(2*t(i)) -0.5*sin(0.5*t(i))]';
% 8-shaped
    %q_r{i} = [sin(t(i)) sin(2*t(i)) cos(0.5*t(i))]';
    q_r{i} = [sin(t(i)) sin(2*t(i)) cos(0.5*t(i))]';
    q_r_dot{i} = [cos(t(i)) 2*cos(2*t(i)) -0.5*sin(0.5*t(i))]';
    x_r{i} = [q_r{i}' q_r_dot{i}']';
    % dxr = hr(xr)
    x_r_dot{i} = [cos(t(i)) 2*cos(2*t(i)) -0.5*sin(0.5*t(i)) -sin(t(i)) -4*sin(2*t(i)) -0.25*cos(0.5*t(i))]';
...
%% Update NN Weight
    ...
    delta =  (d)'*R*(d)+ z{i}'*[Q zeros(6); zeros(6) zeros(6)]*z{i} + 1/4*W_a{i}'*D_1*W_a{i} + W_c{i}'*(sigma);
    ...
    W_a{i+1} = W_a{i} + Step*(-eta_a1*(W_a{i} - W_c{i})-eta_a2*W_a{i}+eta_a/4*D_1*W_a{i}...
              *sigma_ngang'/((1+v*sigma'*gamma_upper{i}*sigma)^(1/2))*W_c{i});
end

%% Plot
    q = cell2mat(q);
    q_r = cell2mat(q_r);
    W_a_plot = cell2mat(W_a);
    W_c_plot = cell2mat(W_c);
    
     V_z=cell2mat(V_z);
 writematrix(V_z,'V_Robust.csv');
%% Plot X,Y,Theta
    figure;
    subplot(3,1,1);
    plot(t,q(1,:));
...
%% Plot Wa, Wc
    ...
    tau = cell2mat(tau);
    writematrix(tau, 'tau_Robust.csv');

     writematrix(q,'eta_Robust.csv');
 writematrix(nominal_E,'consumption_Robust.csv');
% %% Plot control signal
%     tau = cell2mat(tau);
%     figure;
%     plot(t,tau(1,2:length(t)+1));
%     hold on
%     plot(t,tau(2,2:length(t)+1));
%     plot(t,tau(3,2:length(t)+1));
%     hold off
%     legend('Torque 1','Torque 2','Torque 3');
%     title('Control signal');
% %% 10 last seconds
%     figure;
%     plot(q(1,length(t)-10000:end),q(2,length(t)-10000:end));
%     hold on
%     plot(q_r(1,length(t)-10000:end),q_r(2,length(t)-10000:end));
%     title('Trajectory');
%     legend('Real Trajectory','Reference Trajectory');
%     hold off
% 
...
% % writematrix(q,'eta_Robust.csv');
% % writematrix(tau,'tau_Robust.csv');
% % writematrix(W_a_plot,'robust_Wa.csv');
% % figure
% % plot(t,V_z)
% % ylim([0 100])
% writematrix(V_z,'V_Robust.csv');
